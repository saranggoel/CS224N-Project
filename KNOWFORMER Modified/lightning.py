...
class KnowformerLightningModule(pl.LightningModule):
    ...
    def compute_loss(self, score, batched_data):
        ...
        # print(batched_data)
        edge_type = batched_data['r_index']  
        target_edge_types = torch.tensor([2, 3], device=edge_type.device) 
        enhanced_weight = 3 
        ...
        if self.hparams.loss_fn == 'bce':
            ...
            # enhanced_mask = edge_type.unsqueeze(1).eq(target_edge_types.unsqueeze(0)).any(dim=1).float()
            enhanced_mask = torch.isin(edge_type, target_edge_types).float()
            weights[:, 1:] *= (1 + enhanced_mask.unsqueeze(1) * (enhanced_weight - 1))
            
            loss = (loss * weights).sum()

        else:
            # Compute the base cross-entropy loss
            loss = F.cross_entropy(score, positive_index.view(-1), reduction='none')
            enhanced_mask = torch.isin(edge_type, target_edge_types).float()
            weights = 1 + enhanced_mask * (enhanced_weight - 1)
